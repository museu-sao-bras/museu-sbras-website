...
const categories = { temporary: 'Temporary' };
function getExhibitions() {
  const data = localStorage.getItem(EXHIBITIONS_KEY);
  return data ? JSON.parse(data) : [];
}
export default function TemporaryExhibitions() {
  const exhibitions = getExhibitions().filter(ex => ex.category === 'temporary');
  const [activeIdx, setActiveIdx] = useState(0);

  if (exhibitions.length === 0) {
    return (
      <div className="container mx-auto min-h-screen pt-[120px] pb-16 flex flex-col items-center justify-center">
        <h1 className="text-4xl font-bold mb-8 text-center">Temporary Exhibitions</h1>
